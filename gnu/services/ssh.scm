...
When @var{daemonic?} is true, @command{lshd} detaches from the controlling
terminal and starts to log its output to syslogd, unless one sets
@var{syslog-output?} to false.  Obviously, it will also make lsh-service
...
The other options should be self-descriptive."
  ...
  (with-monad %store-monad
    (return (service
             ...
             (requirement #~(if (and #$daemonic? #$syslog-output?)
                                '(networking syslogd)
                                '(networking)))
